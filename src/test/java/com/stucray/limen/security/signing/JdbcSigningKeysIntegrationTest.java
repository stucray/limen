...
@SpringBootTest
@DisplayName("JdbcSigningKeys (per-tenant RSA key storage)")
class JdbcSigningKeysIntegrationTest {

    @Autowired JdbcSigningKeys signingKeys;
    ...
    void getJwkSetReturnsPublicKeysForTenant() throws Exception {
        ...
        JWKSet jwkSet = signingKeys.getJwkSet(tenant.id());
    ...
    void deleteForTenantRemovesAllRows() {
        ...
        signingKeys.deleteForTenant(tenant.id());
    ...
    void getActiveSigningKeyReturnsNullWhenNoneExists() {
        assertThat(signingKeys.getActiveSigningKey(tenant.id())).isNull();
    }
    ...
    void rotateForTenantSwapsActiveAndRetiresPrevious() {
        ...
        SigningKeyLifecycle.RotationOutcome outcome = signingKeys.rotateForTenant(tenant.id());
    ...
    void rotateForTenantThrowsWhenNoActiveKey() {
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->
            signingKeys.rotateForTenant(tenant.id()))
    ...
    void pruneDeletesGraceExpiredRetiredRowsAndReturnsTheirIdentities() {
        ...
        List<SigningKeyLifecycle.PrunedKey> pruned = signingKeys.pruneRetiredOlderThan(Duration.ofHours(24));
    ...
    void pruneNeverTouchesActiveRows() {
        ...
        List<SigningKeyLifecycle.PrunedKey> pruned = signingKeys.pruneRetiredOlderThan(Duration.ofHours(1));
