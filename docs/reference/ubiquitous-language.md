...
| **Loopback Port Wildcarding** | A registered loopback redirect URI (`127.0.0.1/8` or `[::1]`) matches any port on the request side, applied identically at `/oauth2/authorize` (Spring AS built-in, RFC 8252 §7.3) and `/connect/logout` (Limen's `LoopbackAwareOidcLogoutValidator`). `localhost` is NOT a loopback host for matching purposes | "any-port matching", "127.0.0.1 wildcard" |
