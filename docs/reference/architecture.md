...
- **`full_name` is an optional free-form name** added by V14, exposed as the OIDC `name` claim when a client is granted the `profile` scope (see §4.3 — `OidcScopeClaims`). Nullable: existing users keep `null` and the `name` claim is simply omitted from their ID token + `/userinfo` response until they fill it in. Single field rather than first/last because OIDC's `name` claim is itself a free-form display string.
...
A third shape that lives in `oauth2.sas` is `OidcScopeClaims` — the single source of truth for which OIDC standard scopes Limen actually honours. The discovery customizer in `SasConfig` reads it to advertise `scopes_supported` and `claims_supported`; the ID-token branch of the JWT customizer reads it to populate the matching claims when the scope is in the granted set. The default Spring Authorization Server userinfo mapper filters ID-token claims by granted scope using the standard OIDC mapping, so `/userinfo` stays in lockstep without a separate customizer. Today this advertises `openid` + `email` + `profile` — `email` populates `email`/`email_verified` from the `User` row, `profile` populates `name` from `User.fullName` (omitted entirely when null rather than serialised as `name: null`). Make-it-honest fix for #300 (PRD #301).

A fourth shape lives in `oauth2.sas` alongside the adapters but is also distinct: `SasServerErrorTranslationFilter`. It's a servlet `Filter` registered at the head of the SAS `SecurityFilterChain` by `SasConfig`. It catches any uncaught `RuntimeException` thrown by downstream SAS endpoint filters and rewrites the response as `500 application/json {"error":"server_error", ...}` — the RFC 6749 §5.2 shape that RFC-compliant OAuth2 clients (including Spring Security's `OAuth2ErrorResponseErrorHandler`) can parse. `OAuth2AuthenticationException` is intentionally allowed to propagate so SAS's per-endpoint failure handlers still write the canonical `invalid_grant` / `invalid_client` / etc. responses. Without this filter, a fault like `JwtEncodingException` (raised when a tenant signing key fails to unwrap) escapes `FilterChainProxy`, is forwarded to `/error`, hits the catch-all chain's `denyAll`, and emerges as `403 [no body]` — the symptom that surfaced as #293. `SasServerErrorTranslationFilterTest` pins the unit contract; `Issue293ConfidentialPkceTokenIntegrationTest` pins the full Tomcat-backed wiring with a deliberately-broken `JWKSource`.

A fifth shape is `LoopbackAwareOidcLogoutValidator` — a `Consumer<OidcLogoutAuthenticationContext>` swapped into SAS's `OidcLogoutAuthenticationProvider` via the `oidc(...).logoutEndpoint(...).authenticationProviders(...)` configurer DSL in `SasConfig`. It wraps SAS's default post-logout-redirect-URI validator: if the default rejects AND the requested URI's host is a loopback address (`127.0.0.1/8` or `[::1]`), retries the match by substituting the requested port into each registered URI and string-comparing. Mirrors SAS's existing `/oauth2/authorize` loopback branch ([SAS #243](https://github.com/spring-projects/spring-authorization-server/issues/243), shipped in SAS 0.1.1) on `/connect/logout`, which SAS itself does not relax — restoring `/authorize` ↔ `/logout` symmetry for OAuth2 clients that rotate between local-dev ports. The relaxation is grounded in RFC 8252 §7.3 (BCP 212); `localhost` is intentionally NOT a loopback host per RFC 8252 §8.3 and [SAS #651](https://github.com/spring-projects/spring-authorization-server/issues/651). Scheme is preserved transitively by the port-substituted string compare — HTTPS-registered + HTTPS-requested matches, but a scheme mismatch fails. The validator hookup point on `OidcLogoutAuthenticationProvider` was added in [SAS 1.4](https://github.com/spring-projects/spring-authorization-server/issues/1693) (Oct 2024). `LoopbackAwareOidcLogoutValidatorTest` pins the decision logic; `OidcLogoutLoopbackIntegrationTest` proves the wiring at the HTTP boundary. PRD #316.

A sixth shape is `OidcLogoutErrorResponseHandler` — an `AuthenticationFailureHandler` set on the logout endpoint via the `oidc(...).logoutEndpoint(...).errorResponseHandler(...)` configurer DSL in `SasConfig`. SAS's default logout failure handler calls `response.sendError(400, …)`, which the servlet container ERROR-dispatches to `/error`; in Limen's multi-chain setup no chain owns `/error`, so it lands on the catch-all `DefaultSecurityConfig` chain and is rejected by its `anyRequest().denyAll()` — the browser receives a bodyless `403` it renders as a hard error page (#324). This handler writes the `OAuth2Error` directly as `400 application/json` instead, keeping the response self-contained and renderable. It's the same `/error`-forward gap `SasServerErrorTranslationFilter` (#293) closed for thrown `RuntimeException`s, but for the `sendError` path that filter deliberately leaves to the per-endpoint handler. Only the active-session logout path exposes it in practice: SAS's `sub`/`sid`-claim validation is the only logout validation that can fail once `post_logout_redirect_uri` is already known-good, and that branch runs only when a principal is present. `OidcLogoutErrorResponseHandlerTest` pins the unit contract; `OidcRpInitiatedLogoutJourneyUiIT` pins the full real-browser wiring (success + renderable-failure).
...
2. `resumeOAuth2Authorize()` — replays a pending `/oauth2/authorize` request, tenant-prefixing the URL under `/t/{slug}/`. It consults the in-session `HttpSessionRequestCache` SavedRequest **first**; when that is absent (the session was evicted while the User sat on the login page) it falls back to a durable record referenced by the login URL's `?ref=` (see **Durable authorize replay** below).
3. `tenantHome()` — terminal default: redirect to the surface's home (the neutral end-user page on the OAuth2 surface — issue #327).
...
**Durable authorize replay.** The in-session SavedRequest lives only as long as the 30-minute HTTP session, so an unauthenticated `/oauth2/authorize` that bounces to `/t/{slug}/login` and then idles past the timeout would otherwise lose the pending flow. `PendingAuthorizeStore` (in `auth.login`, `pending_authorize` table) stashes the tenant-stripped authorize request under an opaque, high-entropy, single-use, tenant-bound reference (60-minute TTL); the SAS-chain `TenantLoginUrlAuthenticationEntryPoint` appends that reference to the login redirect as `?ref=`, the login form carries it through the POST, and `resumeOAuth2Authorize()` consumes it as the fallback above. The stashed parameters are non-secret and SAS re-validates `redirect_uri` at `/authorize`, so this is a resume-convenience mechanism, not a trust boundary; expiry degrades softly to the neutral end-user home. `PendingAuthorizeSweep` deletes expired rows hourly under a ShedLock guard.
...
The 10 OAuth2 + auth/login journey tests carry `@Tag("cross-browser")`. `PlaywrightExtension` reads `-Dplaywright.browser=chromium|chrome|webkit|firefox` (default `chromium`) so local `mvn verify` runtime is unchanged, while CI's `cross-browser-ui.yml` runs the tagged subset under real stable Google Chrome (the load-bearing engine for catching #285-class bundled-Chromium gaps) — see §4.14. WebKit and Firefox remain reachable via `-Dplaywright.browser=...` for ad-hoc local triage; the CI matrix only runs Chrome.
...
CI is a single GitHub Actions workflow (`.github/workflows/ci.yml`) with one `verify` job on push and PR to `main`. The job sets up JDK 26 (BellSoft Liberica, matching the JDK that Paketo buildpacks install into the production image — so CI and prod run the same OpenJDK distribution by default; configurable via `stucray/workflows`'s `java_distribution` input as of `v1.5.0`), runs `./mvnw -B -ntp verify`, and uploads the PMD bundle (30-day retention) and — on failure — the JaCoCo HTML report (14-day retention). The `LIMEN_SECURITY_KEK` is supplied as a GitHub Actions secret. Docs-only changes (`**.md`, `docs/**`, `LICENSE`, `.github/ISSUE_TEMPLATE/**`, `.github/PULL_REQUEST_TEMPLATE.md`) are skipped via `paths-ignore`; the same filter is applied to `publish-image.yml` so docs commits don't trigger a no-op image rebuild. A `workflow_dispatch` trigger on `ci.yml` provides a manual run lever when the path filter would otherwise skip a run you want to force.

A second CI workflow (`.github/workflows/cross-browser-ui.yml`) runs the `@Tag("cross-browser")` Playwright journey tests under real stable Google Chrome (Playwright's `chrome` channel, not the bundled Chromium) on push and PR to `main`. The matrix logic lives in the `ui-cross-browser-matrix` reusable workflow at `stucray/workflows@v1`; `ci.yml` (default bundled Chromium) and this workflow together cover both Chrome shapes. The motivation is engine-specific drift the bundled Chromium can't catch — most notably real-Chrome-only behaviour like the `/.well-known/appspecific/com.chrome.devtools.json` workspace-folders probe that surfaced bug #285. WebKit and Firefox were part of the original matrix (PR #292) and were dropped 2026-05-21 after 4 days with zero engine-divergent findings — `webkit` and `firefox` can be re-enabled via the workflow's `browsers` input if a future change introduces engine-sensitive code (WebAuthn, a JS framework, complex CSS).
