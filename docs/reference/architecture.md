...
- **No custom metric counters yet.** Stock auto-instrumentation is wired (HTTP, JVM, JDBC, GC, threads) via the Spring Boot 4 OpenTelemetry starter pushing OTLP to a `grafana/otel-lgtm` dev container; tenant attribution is set on spans + log MDC by `observability.TenantObservabilityFilter`. The named auth counters in v3.5 below (login success/failure, token issuance, key rotation) are the remaining work. See `dev/observability/README.md`.
